...
<body>
  <nav class="navbar">
    ...
    <ul class="nav-list">
      ...
      <li><a class="active" href="laundry_page.php"><i class="fas fa-tshirt"></i> Laundry</a></li>
  ...
  <div class="container">
    <section class="module">
      <h2><i class="fas fa-tshirt"></i> New Laundry Order</h2>
      <form class="module-form" data-endpoint="laundry.php" data-action="newOrder">
        <div class="form-grid">
          <label>Guest<input type="text" name="guest" placeholder="Guest name"></label>
          <label>Amount<input type="number" step="0.01" name="amount" placeholder="300"></label>
        </div>
    ...
    <section class="module">
      ...
      <form class="module-form" data-endpoint="laundry.php" data-action="completeOrder">
        <div class="form-grid">
          <label>Order #<input type="text" name="order" placeholder="LD-1234"></label>
        </div>
        <button type="submit" class="button">Complete</button>
      </form>
    </section>
    <section class="module">
      <h2>Recent Laundry Actions</h2>
  ...
  async function submitModuleForm(form){
    ...
    const module='Laundry';
    const ref=(j.data&&(j.data.order||('LD-'+Math.floor(Math.random()*9000+1000))))||(j.action||'Action');
    const amt=j.data&&j.data.amount!=null?('₱'+Number(j.data.amount).toFixed(2)):'—';
    const status=j.action==='completeOrder'?'Completed':'Queued';
  ...
  async function loadActivities(){
    try{
      const r = await fetch('activities.php?action=list&module=Laundry&limit=10');
      const j = await r.json();
      ...
      j.data.rows.forEach(row=>{
        ...
        const st = row.action==='completeOrder'?'Completed':'Queued';
        const tr=document.createElement('tr');
        tr.innerHTML = `<td data-label=\"Date\">${date}</td><td data-label=\"Module\">Laundry</td><td data-label=\"Reference\">${row.reference||row.action}</td><td data-label=\"Amount\">${amt}</td><td data-label=\"Status\">${st}</td>`;
        tbody.prepend(tr);
      });
    }catch(e){}
  }
