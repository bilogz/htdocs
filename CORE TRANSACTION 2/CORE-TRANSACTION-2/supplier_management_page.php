...
<body>
  <nav class="navbar">
    ...
    <ul class="nav-list">
      ...
      <li><a class="active" href="supplier_management_page.php"><i class="fas fa-truck"></i> Supplier Management</a></li>
  ...
  <div class="container">
    <section class="module">
      <h2><i class="fas fa-truck"></i> Create Purchase Order</h2>
      <form class="module-form" data-endpoint="supplier_management.php" data-action="createPO">
        <div class="form-grid">
          <label>Vendor<input type="text" name="vendor" placeholder="ABC Supplies"></label>
          <label>Amount<input type="number" step="0.01" name="amount" placeholder="5000"></label>
        </div>
        <button type="submit" class="button">Create PO</button>
      </form>
    </section>
    <section class="module">
      <h2>Receive Delivery</h2>
      <form class="module-form" data-endpoint="supplier_management.php" data-action="receiveDelivery">
        <div class="form-grid">
          <label>PO #<input type="text" name="po" placeholder="PO-1234"></label>
        </div>
        <button type="submit" class="button">Receive</button>
      </form>
    </section>
    <section class="module">
      <h2>Recent Supplier Actions</h2>
  ...
  function addRow({date,module,ref,amount,status}){
    ...
    tr.innerHTML=`<td data-label="Date">${date}</td><td data-label="Module">${module}</td><td data-label="Reference">${ref}</td><td data-label="Amount">${amount}</td><td data-label="Status">${status}</td>`; tbody.prepend(tr);
  }
  async function submitModuleForm(form){
    ...
    const module='Supplier Management';
    const ref=(j.data&&(j.data.po||j.data.dr||('PO-'+Math.floor(Math.random()*9000+1000))))||(j.action||'Action');
    const amt=j.data&&j.data.amount!=null?('₱'+Number(j.data.amount).toFixed(2)):'—';
    const status=j.action==='receiveDelivery'?'Received':'Issued';
  ...
  async function loadActivities(){
    try{
      const r = await fetch('activities.php?action=list&module=Supplier%20Management&limit=10');
      const j = await r.json();
      ...
      j.data.rows.forEach(row=>{
        ...
        const st = row.action==='receiveDelivery'?'Received':'Issued';
        const tr=document.createElement('tr');
        tr.innerHTML = `<td data-label="Date">${date}</td><td data-label="Module">Supplier Management</td><td data-label="Reference">${row.reference||row.action}</td><td data-label="Amount">${amt}</td><td data-label="Status">${st}</td>`;
        tbody.prepend(tr);
      });
    }catch(e){}
  }
